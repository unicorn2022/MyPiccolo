...
namespace Piccolo {
    ...
    public:
        ...
        Matrix4x4 transpose() const {
            return Matrix4x4(m_mat[0][0], m_mat[1][0], m_mat[2][0], m_mat[3][0], m_mat[0][1], m_mat[1][1], m_mat[2][1], m_mat[3][1], m_mat[0][2], m_mat[1][2], m_mat[2][2], m_mat[3][2], m_mat[0][3], m_mat[1][3], m_mat[2][3], m_mat[3][3]);
        }
        // 获取余子式
        ...
        static Matrix4x4 rotationMatrix(Vector3 normal) {
            ...
            if (fabs(normal.z) > 0.999f) up = Vector3(0, 1, 0);
        ...
        bool hasScale() const {
            ...
            if (!Math::realEqual(t, 1.0, (float)1e-04)) return true;
