/**
 * @file render_type.h
 * @brief 定义渲染相关的数据类型, RHI: Render Hardware Interface
 */
#pragma once
...
#define RHI_FALSE false
    ...
    /**
     * @brief RHI共享类型: exclusive, concurrent
     */
    ...
    /**
     * @brief RHI命令缓冲级别: primary, secondary
     */
    ...
    /**
     * @brief RHI采样数量标志位: 1, 2, 4, 8, 16, 32, 64
     */
    ...
    /**
     * @brief RHI纹理存储方式: optimal, linear, drm_format_modifier_ext
     */
    ...
    /**
     * @brief RHI纹理视图类型: 1D, 2D, 3D, Cube, 1DArray, 2DArray, CubeArray
     */
    ...
    /**
     * @brief RHI附件加载操作: Load, Clear, DontCare, NoneExt
     */
    ...
    /**
     * @brief RHI附件存储操作: Store, DontCare, NoneKHR, NoneQCOM, NoneEXT
     */
    ...
    /**
     * @brief RHI比较操作: Never, Less, Equal, LessOrEqual, Greater, NotEqual, GreaterOrEqual, Always
     */
    ...
    /**
     * @brief RHI描述符(shader中的uniform变量)类型
     */
    ...
    /**
     * @brief RHI过滤方式: Nearest, Linear, CubicIMG, CubicEXT
     */
    enum RHIFilter : int {
    ...
    /**
     * @brief RHI正面方向定义: CounterClockwise, Clockwise
     */
    ...
    /**
     * @brief RHI物理设备类型: Other, IntegratedGPU, DiscreteGPU, VirtualGPU, CPU
     */
    ...
    /**
     * @brief RHI渲染管线绑定点: Graphics, Compute, RayTracingKHR, SubpassShadingHUAWEI, RayTracingNV
     */
    ...
    /**
     * @brief RHI多边形模式: Fill, Line, Point, FillRectangleNV
     */
    ...
    /**
     * @brief RHI采样器寻址模式: Repeat, MirroredRepeat, ClampToEdge, ClampToBorder, MirrorClampToEdge, MirrorClampToEdgeKHR
     */
    ...
    /**
     * @brief RHI采样器Mipmap模式: Nearest, Linear
     */
    ...
    /**
     * @brief RHI顶点输入速率: Vertex, Instance
     */
    ...
    /**
     * @brief RHI剔除模式标志位: None, Front, Back, FrontAndBack
     */
    ...
    /**
     * @brief RHI子通道内容: Inline, SecondaryCommandBuffers
     */
    ...
    /**
     * @brief RHI Fence创建标志位 (CPU 和 GPU 之间的同步)
     */
    ...
    /**
     * @brief RHI命令缓冲使用标志位: OneTimeSubmit, RenderPassContinue, SimultaneousUse
     */
    ...
    /**
     * @brief RHI默认采样器类型: Linear, Nearest
     */
...
namespace MyPiccolo {
     ...
     * @param m_height 高度
     * @param m_depth 深度
     * @param m_mip_levels mipmap层级
     * @param m_array_layers 数组层级
     * @param m_pixels 数据指针
     * @param m_format 格式
     * @param m_type 类型
     */
    ...
    public:
        ...
        uint32_t m_mip_levels {0};   // mipmap层级
        uint32_t m_array_layers {0}; // 数组层级
    ...
    /**
     * @brief 网格体顶点绑定数据
     * @param m_index0_3 骨骼索引
     * @param m_weight0_3 骨骼权重
     */
    ...
    /**
     * @brief 网格体资源描述
     * @param m_mesh_file 网格体文件
     */
    struct MeshSourceDesc {
        std::string m_mesh_file; // 网格体文件
    ...
    /**
     * @brief 材质资源描述
     * @param m_base_color_file 基础颜色贴图
     * @param m_metallic_roughness_file 金属粗糙度贴图
     * @param m_normal_file 法线贴图
     * @param m_occlusion_file 遮挡贴图
     * @param m_emissive_file 自发光贴图
     */
    struct MaterialSourceDesc {
        std::string m_base_color_file;         // 基础颜色贴图
        std::string m_metallic_roughness_file; // 金属粗糙度贴图
        std::string m_normal_file;             // 法线贴图
        std::string m_occlusion_file;          // 遮挡贴图
        std::string m_emissive_file;           // 自发光贴图
    ...
    /**
     * @brief 静态网格体数据
     * @param m_vertex_buffer 顶点缓冲
     * @param m_index_buffer 索引缓冲
     */
    struct StaticMeshData {
        std::shared_ptr<BufferData> m_vertex_buffer; // 顶点缓冲
        std::shared_ptr<BufferData> m_index_buffer;  // 索引缓冲
    };

    /**
     * @brief 渲染网格体数据
     * @param m_static_mesh_data 静态网格体数据
     * @param m_skeleton_binding_buffer 骨骼绑定缓冲
     */
    struct RenderMeshData {
        StaticMeshData              m_static_mesh_data;        // 静态网格体数据
        std::shared_ptr<BufferData> m_skeleton_binding_buffer; // 骨骼绑定缓冲
    };

    /**
     * @brief 渲染材质数据
     * @param m_base_color_texture 基础颜色贴图
     * @param m_metallic_roughness_texture 金属粗糙度贴图
     * @param m_normal_texture 法线贴图
     * @param m_occlusion_texture 遮挡贴图
     * @param m_emissive_texture 自发光贴图
     */
    struct RenderMaterialData {
        std::shared_ptr<TextureData> m_base_color_texture;         // 基础颜色贴图
        std::shared_ptr<TextureData> m_metallic_roughness_texture; // 金属粗糙度贴图
        std::shared_ptr<TextureData> m_normal_texture;             // 法线贴图
