highp vec3 V = normalize(camera_position - in_world_position); // 观察方向
highp vec3 R = reflect(-V, N);                                 // 反射方向

highp vec3 origin_samplecube_N = vec3(N.x, N.z, N.y);
highp vec3 origin_samplecube_R = vec3(R.x, R.z, R.y);

// 通过插值计算F0
//   当金属度接近0时, F0接近(0.04, 0.04, 0.04)
//   当金属度接近1时, F0接近(basecolor)
...
for (highp int light_index = 0; light_index < int(point_light_num) && light_index < m_max_point_light_count; ++light_index) {
    ...
    highp vec3 point_light_radius   = scene_point_lights[light_index].radius;

    // 入射光方向
    highp vec3 L   = normalize(point_light_position - in_world_position);
    highp vec3 NoL = min(dot(N, L), 1.0);

    // 点光源衰减
    highp float distance             = length(point_light_position - in_world_position);                          // 距离
    highp float distance_attenuation = 1.0 / (distance * distance + 1.0);                                         // 距离衰减
    highp float radius_attenuation   = 1.0 / ((distance * distance) / (point_light_radius * point_light_radius)); // 半径衰减
    highp float light_attenuation    = radius_attenuation * distance_attenuation * NoL;                           // 光照衰减
    ...
    if (light_attenuation > 0.0) {
        ...
        highp float shadow;
        {
            ...
            // 观察点所在的纹理贴图index, 使用sign防止歧义(z<0 => +0; z>0 => +1)
...
/* 方向光 */
{
    ...
    if (NoL > 0.0) {
        ...
        highp float current_length = position_ndc.z;

        shadow = (closest_depth >= current_length) ? 1.0f : -1.0f;
    }
